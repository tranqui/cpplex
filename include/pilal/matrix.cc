...
#ifndef PILAL_MATRIX_IMPL_H
#define PILAL_MATRIX_IMPL_H
...
namespace pilal {
    ...
    Scalar& Matrix::at (int r, int c) {
        if ( r >= rows || c >= columns)
            throw(IndexOutOfBoundException());

        //Scalar& dest = values->at(r * columns + c);
        //return Matrix::storage_accessor(dest, *this);
        return values->at(r * columns + c);
    }

    Scalar const& Matrix::at (int r, int c) const {
        if ( r >= rows || c >= columns)
            throw(IndexOutOfBoundException());
    ...
    Matrix& Matrix::operator*=(Matrix const& m) {
        ...
        // Swap contents
        std::swap( values->contents, r.values->contents );
        rows = r.rows;
        columns = r.columns;
    ...
    AnonymousMatrix Matrix::operator*(AnonymousMatrix m) const {
        ...
        // Swap pointers
        std::swap( m.values->contents, r.values->contents );                         // Keep m's counter
        m.rows = r.rows;
        m.columns = r.columns;
    ...
    double Matrix::space() const {
        return (rows * columns * sizeof(Scalar) * 0.000000954);
    }

    // Aux
    bool Matrix::more_equal_than (Scalar value, Scalar tol = 0.0000000000000001) const {
        for (int i = 0; i < rows; ++i)
            for (int j = 0; j < columns; ++j)
    ...
    bool Matrix::less_equal_than (Scalar value, Scalar tol = 0.0000000000000001) const {
    ...
    void Matrix::swap_columns(int r, int w) {
        ...
        for (int i = 0; i < rows; ++i)
            std::swap( values->contents->at(i * columns + r), values->contents->at(i * columns + w) );
    ...
    void Matrix::swap_rows(int r, int w) {
        ...
        for (int i = 0; i < columns; ++i)
            std::swap( values->contents->at(r * columns + i), values->contents->at( w * columns + i) );
    ...
    bool Matrix::is_identity(Scalar tol) const {
    ...
    void Matrix::set_determinant(Scalar d) {
    ...
    AnonymousMatrix Matrix::gaussian_elimination() {
        ...
        while (j < columns && pivot < rows) {
            ...
            Scalar max = r(column_max_position,j);
    ...
    void Matrix::get_lupp(Matrix& l, Matrix& u, Matrix& p, PermutationFormat pf = PF_MATRIX) const {
        ...
        for (int j = 0; j < columns; ++j) {
            ...
            Scalar max = u(column_max_position,j);
            ...
            // of l and tem
            for (int i = 0; i < rows; ++i) {
                register Scalar inv_product = l(i,j);   // because tem(j,0) == 1
    ...
    void Matrix::solve(Matrix& x, Matrix const& b) const {
        ...
        for (int i = 0; i < rows; ++i) {
            register Scalar dot_product = pb(i);
            for (int j = 0; j < i; ++j) {
    ...
    /*
      Matrix::storage_accessor
    */
    Matrix::storage_accessor::storage_accessor(Scalar& dest, Matrix& parent) :
        dest(dest),
        parent(parent) {
    }

    Matrix::storage_accessor::operator Scalar const& () const {
        return dest;
    }
    ...
    Matrix::storage_accessor& Matrix::storage_accessor::operator=(Scalar const& new_value) {
    ...
    /*
      Matrix::storage
      ===============
      Reference counted vector of Scalars.

    */

    Matrix::storage::storage(int size) :
        contents(new std::vector< Scalar>(size)),
        counter(1) {
    }

    Matrix::storage::storage(Matrix::storage& s) :
        contents(new std::vector< Scalar>(*(s.contents))),
        counter(1) {
    }

    Matrix::storage::storage(int size, Scalar value) :
        contents(new std::vector< Scalar>(size, value)),
        counter(1) {
    }

    Scalar& Matrix::storage::at(int pos) {
    ...
    */
    ...
    AnonymousMatrix::AnonymousMatrix(const AnonymousMatrix& m) : Matrix(m.rows, m.columns) {
        ...
        std::swap(Matrix::values->contents, m.values->contents);
    ...
    bool tol_equal(Scalar n, Scalar m, Scalar tol = 0.0000000000000001) {
        if ( abs( n - m ) > tol )
            return false;
        return true;
    }
